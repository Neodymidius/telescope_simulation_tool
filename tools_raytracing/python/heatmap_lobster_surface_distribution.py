    ...
    with open(path) as f:
        for line in f:
            ...
            if key not in reflections:
                reflections[key] = []
            # NOTE: This variant uses columns 3 and 4 for x,y
            hit = [int(parts[0]), float(parts[3]), float(parts[4])]
...
def main():
    ...
    # Optional palette of colormaps to rotate panels (kept from your script; can be ignored)
    ...
    for path in file_paths:
        ...
        for key_chunk in chunked(keys_sorted, 9):
            ...
            for idx, key in enumerate(key_chunk):
                ...
                # Optionally rotate colormaps per panel; fallback to 'hot'
                cmap = plot_color_gradients[idx % len(plot_color_gradients)]
            ...
            fig.suptitle(f"Distribution of rays per reflection on MPO", y=0.995)
            plt.tight_layout()
            plt.show()
            page_num += 1

        # Scatter plot overview (kept, with safe color cycling)
