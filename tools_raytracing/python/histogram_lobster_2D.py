...
def plot_sensor(ax, sensor, extent, title, cmap='hot'):
    im = ax.imshow(sensor, origin='upper',
                   extent=extent,
                   cmap=cmap,
                   norm=colors.LogNorm(vmin=1e-1, vmax=sensor.max()))
...
def main():
    ...
    for idx, path in enumerate(file_paths):
        ...
        summed = sensor[120:135]
        flat = np.sum(summed, axis=0) - 1.5
        flat_normalized = flat / np.max(flat)

        plt.plot(flat_normalized )

    plt.xlabel("Pixel (Pixel size: 0.44 mm)")
